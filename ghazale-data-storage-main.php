...
function ghazale_ds_welcome_page(){
    ...
    ?>
    <h2><?php _e('Welcome to Data Storage', 'data-storage'); ?> (v2.3.1)</h2>
    <?php
    if($results) {
        ?>
...
function ghazale_ds_enqueue_table_style(){
    wp_enqueue_style('ghazale-ds-enqueue-table-style', plugins_url('css/ds-table-style.css',__FILE__));
}
add_action('wp_enqueue_scripts','ghazale_ds_enqueue_table_style');
/**
            ...
            foreach ($table as $fields_table) {
                ...
                if (!empty($fields_query)) {
                    ...
                    foreach($fields_query as $fields){
                        echo "<tr>";
                        echo "<td><a href='". get_admin_url() ."admin.php?page=ds-forms&ds-del-field-id=".$fields['id'] ."&ds-del-field-table=". $fields_table."#". $fields_table."' style=\"text-decoration: none\" onclick=\"return confirm('".__('Are you sure? (THIS ACTION CANNOT BE UNDONE)','data-storage')."');\"><input type=\"button\" value=\"Delete\" ".(!empty($inputs_query)? 'disabled': '')."></a> </td>";
                ...
                echo "<a href=\"".get_admin_url() ."admin.php?page=ds-forms&ds-total-del-form-table=". $fields_table ."\" onclick=\"return confirm('".__('Are you sure? (THIS ACTION CANNOT BE UNDONE)','data-storage')."');\" ><input type=\"button\" value=\"".__('Totally Delete This Form AND Its Corresponding Inputs Table.','data-storage')."\"></a>";
                echo "</div>";
            }
        }
    }else{
                    ...
                    foreach ($fields as $field) {
                        ...
                        switch ($field['field_type']){
                            ...
                            case "Drop Down":
                                ...
                                $output .= "<option value=\"\" >-- ". __('Select','data-storage') ." --</option>";
    ...
    if(!empty($input_tables)) {
        ...
        foreach ($input_tables as $input_table) {
            foreach ($input_table as $table) {
                ...
                echo "<p><strong>".esc_html('Shortcode: ').'</strong>'.esc_html('[data-storage-entry input="').str_replace(array($tables, '_inputs'), '', $table).'"]</p>';
                ...
                echo "</table>";
                echo "<p><a href=\"" . get_admin_url() . "admin.php?page=ds-form-entries&ds-del-data-input-table=" . $table . "#". $table."\" style=\"text-decoration: none\" onclick=\"return confirm('".__('Are you sure? (THIS ACTION CANNOT BE UNDONE)','data-storage')."');\"><input type=\"button\" value=\"".__('Delete All Data in This Input Table','data-storage')."\"></a></p>";
                echo "<p><a href=\"" . get_admin_url() . "admin.php?page=ds-form-entries&ds-total-del-input-table-and-corresponding-form-table=" . $table . "\" style=\"text-decoration: none\" onclick=\"return confirm('".__('Are you sure? (THIS ACTION CANNOT BE UNDONE)','data-storage')."');\"><input type=\"button\" value=\"".__('Totally Delete This Input Table AND Its Corresponding Form','data-storage')."\"></a></p>";
                echo "</div>";
            }
...
add_shortcode('data-storage-entry', 'ghazale_ds_form_inputs_frontend');
/**
